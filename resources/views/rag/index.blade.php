...
<body class="bg-slate-100 h-screen flex flex-col font-sans overflow-hidden">
    ...
    <div class="flex flex-1 overflow-hidden relative">
        ...
        <aside class="w-1/3 bg-white border-r border-slate-200 p-6 flex flex-col overflow-y-auto">
            ...
            <!-- URL SCRAPER FORM -->
            <form id="url-form" action="{{ route('documents.storeUrl') }}" method="POST"
                class="mb-6 border-2 border-dashed border-slate-300 p-4 rounded-lg bg-slate-50 text-center hover:bg-slate-100 transition">
                @csrf
                <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 text-left">Or Ingest a Website
                    URL</h3>
                ...
                <input type="url" name="url" placeholder="https://example.com/article"
                    class="w-full mb-3 border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                    required />

                <button id="url-btn" type="submit"
                    class="w-full bg-slate-800 hover:bg-slate-900 text-white font-medium py-2 rounded-md text-sm transition">
                    Scrape & Index URL
                </button>
            </form>
            ...
            <div class="space-y-3 flex-1 overflow-y-auto">
                    ...
                    <div class="bg-slate-50 border border-slate-200 p-3 rounded-lg flex justify-between items-center">
                        <div class="truncate pr-2">
                            ...
                            @if ($doc->status === 'processing')
                                <div class="flex items-center gap-1.5 text-yellow-600 mt-1">
                                    ...
                                    <span class="text-xs font-semibold animate-pulse">Processing & Indexing...</span>
                                </div>
                            @elseif($doc->status === 'failed')
                                <span class="text-xs text-red-500 font-semibold mt-1">⚠️ Indexing Failed</span>
                            @else
                                <span class="text-xs text-slate-500 mt-1">{{ $doc->chunks()->count() }} Chunks
                                    Indexed</span>
                            @endif
                        </div>
                ...
                @endforelse
                <p class="text-sm text-slate-400 italic">The application uses queued background jobs for indexing documents. It doesn't have any pooling technology implemented like WebSocket or Server-Sent Events. Please check back later by refreshing the page manually.</p>
            </div>
        </aside>
